/*
 * File: ShowFileContentsByLine.cpp
 * --------------------------
 * This program displays the contents of a file chosen by the user.
 * However, instead of processing the file by character as done
 * in ShowFileContents.cpp, it is done by line
 */
...
string promptUserForFile(ifstream & infile, string prompt) {
    while (true) {
        ...
        infile.clear(); //to ensure failure status indicator in the stream is reset
